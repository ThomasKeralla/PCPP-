...
public class TestCache {
  public static void main(String[] args) throws InterruptedException {
    ...
    // Our own code begin from here
    Factorizer f = new Factorizer();
    for (int c=1; c<=16; c++) {
      final int threadCount = c;
      Mark7(String.format("countParallelN %6d", threadCount),
            i ->  exerciseFactorizer(new Memoizer1<Long,long[]>(f))); // Change the Memoizer here
  ...
  private static void exerciseFactorizer(final Computable<Long, long[]> factorizer) {
    System.out.println(factorizer.getClass());
    final int threadCount = 16;
    final long start = 10_000_000_000L;
    final long range = 2000;
    ...
    for(int i = 0; i < threadCount; i++) {
      ...
      final long from2 = start + range + i * 500;
      final long to2   = start + range + range + i * 500;

      threads[i] = new Thread(() -> {
        try {
          // First range the thread computes
          for(long j = from1; j < to1; j++) {
            factorizer.compute(j);
          }

          // Second range the thread computes
      ...
      threads[i].start();
      System.out.println("Thread started from: " + from2);
